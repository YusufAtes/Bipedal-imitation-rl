...
class BipedEnv(gym.Env):
    ...
    def reset(self,seed=None,test_speed = None, test_angle = None,demo_max_steps = None,
              ground_noise = None, ground_resolution = None, options: dict | None = None):
        ...
        if self.demo_mode == False:
            ...
            p.changeDynamics(self.planeId, -1, lateralFriction=1.0, physicsClientId=self.physics_client)
        else:
    ...
    def biped_reward(self,x,torques):
        ...
        self.imitation_weight = 1 / (1 + np.exp(-10 + 5*self.init_no/12000)) #sigmoid function to scale the imitation weight
        #imitiation_compromise = 1 - self.imitation_weight
        self.alive_weight = 0.2
        self.contact_weight = 0.2
        done = False
        reward = 0

        contact_points = p.getContactPoints(self.robot, self.planeId,physicsClientId=self.physics_client)
        ...
        if left_contact > 2 and right_contact == 0 :
            ...
            reward += self.contact_weight
        elif left_contact > 2 and right_contact > 2:
            reward += self.contact_weight
        # elif left_contact < 1 and right_contact < 1:
        #     reward -= self.contact_weight

        if self.demo_mode == True:
            if self.external_states[2] > 1.5 + np.tan(self.ramp_angle) * self.external_states[1]:
                reward -= 20
                done = True
            ...
            elif self.external_states[2] > 1.25+ np.tan(self.ramp_angle) * self.external_states[1]:
                reward -= self.alive_weight
            elif self.external_states[2] < 0.75+ np.tan(self.ramp_angle) * self.external_states[1]:
                reward -= 20
                done = True
            elif self.external_states[2] < 0.95+ np.tan(self.ramp_angle) * self.external_states[1]:
        ...
        reward += np.exp(-np.abs(current_speed - self.reference_speed))  # reward for maintaining speed newly adjusted
    ...
    def init_state(self):
        if self.demo_mode == False:
            ...
            del self.robot
            self.robot = p.loadURDF("assets/biped2d.urdf", [0,0,init_z+0.02], p.getQuaternionFromEuler([0.,0.,0.]),physicsClientId=self.physics_client)
            p.setJointMotorControlArray(self.robot,[0,1,2,3,4,5,6,7,8], p.VELOCITY_CONTROL, forces=[0,0,0,0,0,0,0,0,0], physicsClientId=self.physics_client)

            p.resetJointState(self.robot, 3, targetValue = rhip_pos, physicsClientId=self.physics_client) 
            p.resetJointState(self.robot, 4, targetValue = rknee_pos, physicsClientId=self.physics_client)
            p.resetJointState(self.robot, 5, targetValue = rankle_pos, physicsClientId=self.physics_client)
            p.resetJointState(self.robot, 6, targetValue = lhip_pos, physicsClientId=self.physics_client)
            p.resetJointState(self.robot, 7, targetValue = lknee_pos, physicsClientId=self.physics_client)
            p.resetJointState(self.robot, 8, targetValue = lankle_pos, physicsClientId=self.physics_client)
        else:
            start_idx = 0
            self.reference_idx = start_idx
            ...
            del self.robot
            self.robot = p.loadURDF("assets/biped2d.urdf", [0,0,init_z], p.getQuaternionFromEuler([0.,0.,0.]),physicsClientId=self.physics_client)
            p.setJointMotorControlArray(self.robot,[0,1,2,3,4,5,6,7,8], p.VELOCITY_CONTROL, forces=[0,0,0,0,0,0,0,0,0], physicsClientId=self.physics_client)
        ...
        self.t1_torso_pos = p.getJointState(self.robot, 2, physicsClientId=self.physics_client)[0]
        self.t1_rhip_pos = p.getJointState(self.robot, 3, physicsClientId=self.physics_client)[0]
        self.t1_rknee_pos = p.getJointState(self.robot, 4, physicsClientId=self.physics_client)[0]
        self.t1_rankle_pos = p.getJointState(self.robot, 5, physicsClientId=self.physics_client)[0]
        self.t1_lhip_pos = p.getJointState(self.robot, 6, physicsClientId=self.physics_client)[0]
        self.t1_lknee_pos = p.getJointState(self.robot, 7, physicsClientId=self.physics_client)[0]
        self.t1_lankle_pos = p.getJointState(self.robot, 8, physicsClientId=self.physics_client)[0]


    def return_state(self):
        
        link_state = p.getLinkState(self.robot, 2,computeLinkVelocity=True,physicsClientId=self.physics_client)          #link index 2 is for torso
        (pos_x,pos_y,pos_z) = link_state[0]                #3D position of the link
        y_vel = link_state[6][1]                           #y velocity of the link

        self.torso_pos = p.getJointState(self.robot, 2, physicsClientId=self.physics_client)[0]
        self.rhip_pos = p.getJointState(self.robot, 3, physicsClientId=self.physics_client)[0]
        self.rknee_pos = p.getJointState(self.robot, 4, physicsClientId=self.physics_client)[0]
        self.rankle_pos = p.getJointState(self.robot, 5, physicsClientId=self.physics_client)[0]
        self.lhip_pos = p.getJointState(self.robot, 6, physicsClientId=self.physics_client)[0]
        self.lknee_pos = p.getJointState(self.robot, 7, physicsClientId=self.physics_client)[0]
        self.lankle_pos = p.getJointState(self.robot, 8, physicsClientId=self.physics_client)[0]

        self.torso_vel = p.getJointState(self.robot, 2, physicsClientId=self.physics_client)[1]
        self.rhip_vel = p.getJointState(self.robot, 3, physicsClientId=self.physics_client)[1]
        self.rknee_vel = p.getJointState(self.robot, 4, physicsClientId=self.physics_client)[1]
        self.rankle_vel = p.getJointState(self.robot, 5, physicsClientId=self.physics_client)[1]
        self.lhip_vel = p.getJointState(self.robot, 6, physicsClientId=self.physics_client)[1]
        self.lknee_vel = p.getJointState(self.robot, 7, physicsClientId=self.physics_client)[1]
        self.lankle_vel = p.getJointState(self.robot, 8, physicsClientId=self.physics_client)[1]
    ...
    def init_noisy_plane(self, noise_level=0.1,ground_resolution= 0.1 ,num_rows=128, num_columns=1024,
        ...
        # p.changeDynamics(
        #     self.planeId, -1,
        #     lateralFriction=0.9,      # rubber-sole on dry concrete
        #     rollingFriction=0.0,      # keep almost zero to discourage wheel-like rolling
        #     spinningFriction=0.003,   # small torsional resistance
        #     restitution=0.0,          # no bounce
        #     contactStiffness=4.0e4,   # [N/m] → ≈ 1 mm penetration under body weight
        #     contactDamping=3.0e2)      # [N·s/m] critical-ish damping
