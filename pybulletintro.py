...
for joint_index in range(p.getNumJoints(robot)):
    ...
    print(f"Joint {joint_index}: {link_name}")
# Print result
for index, name in link_names.items():
    print(f"Link index {index}: {name}")


link_state = p.getLinkState(robot, 2, computeForwardKinematics=True)

torso_g_quat = link_state[1]

print("Torso global orientation (quaternion):", torso_g_quat)
...
for i in range(1000):
    ...
    p.setJointMotorControl2(robot, 4, controlMode=p.TORQUE_CONTROL, force=-200)
    p.stepSimulation()
