...
class BipedEnv(gym.Env):
    ...
    def biped_reward(self,x,torques):
        ...
        self.alive_weight = 0.15
        self.contact_weight = 0.07
        done = False
        reward = 0
        ...
        # else:
        #     reward += 1 * self.alive_weight
        if self.external_states[2] > 1.35 + self.external_states[1]*np.tan(self.ramp_angle):
            reward -=10
            done = True
        elif self.external_states[2] > 1.2+ self.external_states[1]*np.tan(self.ramp_angle):
            reward -= self.alive_weight
        elif self.external_states[2] < 0.85+ self.external_states[1]*np.tan(self.ramp_angle):
            reward -=10
            done = True
        elif self.external_states[2] < 1.+ self.external_states[1]*np.tan(self.ramp_angle):
            reward -= 1 * self.alive_weight
        else:
            reward += 1 * self.alive_weight
        ...
        hip_ref_vel = x[[52,55]]
        reward += 0.5 *np.exp(-0.1*np.linalg.norm(hip_joint_vel - hip_ref_vel))

        knee_joint_vel = x[[29,32]]
        knee_ref_vel = x[[53,56]]
        reward += 0.5 *np.exp(-0.1*np.linalg.norm(knee_joint_vel - knee_ref_vel))

        ankle_joint_vel = x[[30,33]]
        ankle_ref_vel = x[[54,57]]
        reward += 0.5 * np.exp(-0.1*np.linalg.norm(ankle_joint_vel - ankle_ref_vel))

        reward -= 3e-3 * np.mean(np.abs(torques))
        reward += self.external_states[1] / 3
    ...
    def init_state(self):
        ...
        # hip_init = (np.abs(rhip_pos) + np.abs(lhip_pos))/2
        # knee_init = (np.abs(rknee_pos) - np.abs(lknee_pos))/2
    ...
    def return_state(self):
        ...
        self.state[27:34] = [self.torso_vel, self.rhip_vel, self.rknee_vel, self.rankle_vel, self.lhip_vel, self.lknee_vel, self.lankle_vel]

        self.state[34:40] = [self.reference[self.reference_idx+self.t,0], self.reference[self.reference_idx+self.t,1], 
                             self.reference[self.reference_idx+self.t,2], self.reference[self.reference_idx+self.t,3],
                             self.reference[self.reference_idx+self.t,4], self.reference[self.reference_idx+self.t,5]]
        
        self.state[40:46] = [self.reference[self.reference_idx+self.t+10,0], self.reference[self.reference_idx+self.t+10,1],
                                self.reference[self.reference_idx+self.t+10,2], self.reference[self.reference_idx+self.t+10,3],
                                self.reference[self.reference_idx+self.t+10,4], self.reference[self.reference_idx+self.t+10,5]]
        ...
        self.state[46:52] = [self.reference[self.reference_idx+self.t+100,0], self.reference[self.reference_idx+self.t+100,1],
