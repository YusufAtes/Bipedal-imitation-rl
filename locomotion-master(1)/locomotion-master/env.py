...
class Biped2dBullet(gym.Env):
    ...
    def reset(self, disable_gui=True,ramp_angle=0.0,ground_resolution=None,heightfield_data=None):
        if disable_gui:
        ...
        if ground_resolution is None:
            plane_orientation = self.p.getQuaternionFromEuler([ramp_angle*np.pi / 180, 0 , 0],physicsClientId=self.physics_client)
            self.plane_id = self.p.loadURDF("assets/plane.urdf",physicsClientId=self.physics_client, baseOrientation=plane_orientation)
    ...
    def init_noisy_plane(self, noise_level=0.1,ground_resolution= 0.05 ,num_rows=32, num_columns=1024,
        ...
        self.plane_id = self.p.createMultiBody(
            baseMass=0,
