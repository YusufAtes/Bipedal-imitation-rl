...
class BipedEnv(gym.Env):
    def __init__(self,render=False, render_mode= None, demo_mode=False,imitation_weight= 0.9):
        self.p = p
        self.init_no = 0
    ...
    def reset(self,seed=None,test_speed = None, test_angle = None,demo_max_steps = None, 
              ground_noise = None, ground_resolution = None):
    ...
    def biped_reward(self,x,torques):
         ...
         #/ (1 + np.exp(-10 + 5*self.init_no/15000)) #sigmoid function to scale the imitation weight
        ...
        if self.demo_mode == True:
            if self.external_states[2] > 1.3 + np.tan(self.ramp_angle) * self.external_states[1]:
                reward -= 20
                done = True
